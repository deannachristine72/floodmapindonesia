...
<div class="h-full flex flex-col bg-gray-900 border-l border-gray-800">
  ...
  <div class="flex-1 overflow-y-auto px-4 py-4 space-y-5">
    ...
    <!-- Top 10 Kabupaten Terparah (heatmap mode only) -->
        ...
        <div class="space-y-1.5">
          {#each topKota as kota, i}
            <div class="flex items-center gap-2">
              <span class="text-[10px] text-gray-600 font-mono w-5 text-right shrink-0">
                #{i + 1}
              </span>
    ...
    <div class="px-4 py-3 border-t border-gray-800 bg-gray-850">
      <div class="flex items-center justify-between gap-2">
        <div class="min-w-0">
          ...
          <div class="text-sm font-semibold text-teal-400 truncate">{selectedKota.kota_name}</div>
          <div class="text-xs text-gray-400 truncate">{selectedKota.provinsi}</div>
        </div>
        <button
          class="shrink-0 text-gray-400 hover:text-white text-lg leading-none transition-colors"
